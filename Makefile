.PHONY: help install install-dev test test-cov test-allure benchmarks benchmarks-compare bench-report test-compat test-compat-sklearn test-compat-gpytorch test-compat-gpjax test-compat-gpflow lint format clean build upload docs
...
help:
	...
	@echo "  make bench-report            - Generate comparison report from benchmark results"
...
benchmarks:
	...
	pytest benchmarks/base_kernels --benchmark-only -v -s --benchmark-time-unit=ms --benchmark-autosave --benchmark-compare --benchmark-group-by=func --benchmark-sort=name

bench-report:
	$(MAKE) -C KernelBenchmarks report
