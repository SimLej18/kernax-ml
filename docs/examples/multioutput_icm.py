# %% [markdown]
# # Multi-output GP -- Intrinsic Coregionalisation Model (ICM)
#
...
# noise/mean terms, and which kernel/mean is used underneath.
...
from kernax.multioutput import BlockDiagKernel, BlockMean, ICMKernel
...
N_LATENT = 2                # rank of the ICM coregionalisation matrix W, shape (N_OUTPUTS, N_LATENT)
INNER_KERNEL = SEKernel(length_scale=0.8)   # latent kernel shared by every output inside the ICM
...
# %% [markdown]
# ## Build the GP: `ICMKernel + BlockDiagKernel` for the kernel, `BlockMean` for the mean
...
fig.suptitle(
	f"ICM multi-output GP -- ISOTOPIC={ISOTOPIC}, SHARED_OUTPUT_HPS={SHARED_OUTPUT_HPS}"
)
fig.tight_layout()
fig.show()
